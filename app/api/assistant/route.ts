...
export async function POST(req: Request) {
  ...
  if (!process.env.AI_GATEWAY_API_KEY) {
    return new Response(
      "I can help shoppers find best-match deals and date-night bundles by zip code, budget, coupons, pickup links, and delivery-provider links. Add AI_GATEWAY_API_KEY in Vercel Environment Variables to enable live AI responses.",
  ...
  const response = streamText({
    ...
    system:
      "You are DealFinder AI's zip-code savings helper. Be concise, practical, and focused on helping users find the best matching deal by zip code, budget, category, party size, coupons, pickup links, and delivery-provider links. Ordering happens with the restaurant or delivery provider. If a user asks for a deal without a zip code, ask for their zip code before recommending places. When asked for date night, bundle dinner, an activity, and dessert under budget. Do not claim that a store, venue, or restaurant sponsors the app unless the user says an approved partnership exists.",
